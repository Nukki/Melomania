// modal action types
export const HIDE_MODAL = 'HIDE_MODAL';
export const SHOW_MODAL = 'SHOW_MODAL';

// user name types
export const SET_USER = 'SET_USER';
export const UPDATE_SCORE = 'UPDATE_SCORE';
export const CLEAR_USER = 'CLEAR_USER';
export const UPD_USER = 'UPD_USER';

// song types
export const FETCH_SONG_BEGIN = 'FETCH_SONG_BEGIN';
export const FETCH_SONG_SUCCESS = 'FETCH_SONG_SUCCESS';
export const FETCH_SONG_FAILURE = 'FETCH_SONG_FAILURE';
export const CLEAR_SONG = 'CLEAR_SONG';

// answer types
export const CHECK_ANSWER_BEGIN = 'CHECK_ANSWER_BEGIN';
export const CHECK_ANSWER_SUCCESS = 'CHECK_ANSWER_SUCCESS';
export const CHECK_ANSWER_FAILURE = 'CHECK_ANSWER_FAILURE';
export const CLEAR_ANSWER = 'CLEAR_ANSWER';

// leaderbord types
export const FETCH_LEADERS_BEGIN = 'FETCH_LEADERS_BEGIN';
export const FETCH_LEADERS_SUCCESS = 'FETCH_LEADERS_SUCCESS';
export const FETCH_LEADERS_FAILURE = 'FETCH_LEADERS_FAILURE';
